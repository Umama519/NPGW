...
export class NprResendBulkResponseComponent {
  ...
  GetGrid1() {
    debugger;
    const url = `http://localhost:5000/api/NPRResendBulk/LOV?id=${this.loginUser}`;
    this.http.get<any[]>(url).subscribe({
      next: (res: any[]) => {
        if (res && res.length > 0) {
          this.GridData1 = res;
  ...
  GetGrid2(batch: any) {
    debugger;
    const url = `http://localhost:5000/api/NPRResendBulk/BD?userid=${this.loginUser}&batch=${batch}`;
    this.http.get<any[]>(url).subscribe({
      next: (res: any[]) => {
        if (res && res.length > 0) {
          this.GridData2 = res;
  ...
  onProcess(batch: any) {
    ...
    const insertUrl = `http://localhost:5000/api/NPRResendBulk/${batch}`;
...
// Clean invalid rows (optional — agar dono null ho to hatao)
  ...
  submit(rowData: any) {
    ...
    if (this.isadd === 'I') {
      debugger;
      const insertUrl = `http://localhost:5000/api/NPRResendBulk/M`;
        ...
        next: (response: any) => {
          ...
          if (parts.length > 2) {
            ...
            for (let i = 0; i < bulkData.length; i++) {
              ...
              const insertUrl = `http://localhost:5000/api/NPRResendBulk/D`;
