...
@Component({
  selector: 'app-public-nprbulkworkflowcloseprocess-aspx',
    standalone: true,  

  imports: [CommonModule, FormsModule],
  templateUrl: './npr-close-bulk-process.component.html',
  styleUrl: './npr-close-bulk-process.component.css'
})
export class NprCloseBulkProcessComponent {
  ...
  GetGrid() {
    debugger;
    const url = `http://localhost:5000/api/NPRCloseBulk/S?id=${this.loginUser}`;
  ...
  GetGrid1() {
    debugger;
    const url = `http://localhost:5000/api/Action_LOV_/BatchClose`;
    this.http.get<any[]>(url).subscribe({
      next: (res: any[]) => {
        if (res && res.length > 0) {
          this.GridData1 = res;
  ...
  GetGrid2(batch: any) {
    debugger;
    const url = `http://localhost:5000/api/NPRCloseBulk/BD?userid=${this.loginUser}&batch=${batch}`;
    this.http.get<any[]>(url).subscribe({
      next: (res: any[]) => {
        if (res && res.length > 0) {
          this.GridData2 = res;
  ...
  onProcess(batch: any) {
    ...
    const insertUrl = `http://localhost:5000/api/NPRCloseBulk/${batch}`;
  ...
  submit(rowData: any) {
    ...
    if (this.isadd === 'I') {
      debugger;
      const insertUrl = `http://localhost:5000/api/NPRCloseBulk/M`;
      this.http.post(insertUrl, mappedData).subscribe({
        next: (response: any) => {
          ...
          if (parts.length > 2) {
            ...
            if (status === '1') {
              debugger;
              const bulkData: NPRCloseSetup[] = rowData.map((row: any) => {
              const setup = new NPRCloseSetup();
              setup.batch = seq;
              setup.total_records = '';
              setup.processDate = '';
            ...
            for (let i = 0; i < bulkData.length; i++) {
              ...
              const insertUrl = `http://localhost:5000/api/NPRCloseBulk/D`;
  ...
  deleteRecord() {
      ...
      const url = `http://localhost:5000/api/NPRCloseBulk/${this.batch}`
      const options = {
