...
export class PortInoutSummaryReportComponent {
  ...
  Fetch() {
    ...
    const url = `${environment.apiBaseUrl}/api/PortinoutSummaryReport?fromdate=${txt_FromDate}&todate=${txt_ToDate}&userid=${this.loginUser}`;
      ...
      next: (res) => {
        if (res && res.length > 0) {
          if (rhb_Screen === 'S' && res.length < 1000) {
            ...
            this.totalPages = Math.ceil(this.Portinout.length / this.itemsPerPage);
  ...
  export(res: any, file: any) {
    ...
    this.excelService.exportToFile(
      this.GridData, 'Port In-Out Summary Report', {
      nprdate: 'NPR Date',
      recepient: 'Recepient',
      donor: 'Donor',
      portin: 'Port In',
