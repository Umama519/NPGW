...
export class FinanceReportComponent {
  ...
  Fetch() {
    ...
    const url = `${environment.apiBaseUrl}/api/FinanceReport?fromdate=${txt_FromDate}&todate=${txt_ToDate}&userid=${this.loginUser}`;
      ...
      next: (res) => {
        if (res && res.length > 0) {
          if (rhb_Screen === 'S' && res.length < 1000) {
            ...
            this.totalPages = Math.ceil(this.Finance.length / this.itemsPerPage);
  ...
  export(res: any, file: any) {
    ...
    this.excelService.exportToFile(
      this.GridData, 'Finance Report', {
      cash: 'Pamyent Mode',
      nprdate: 'NPR Date',
      npr_no: 'Transaction ID',
