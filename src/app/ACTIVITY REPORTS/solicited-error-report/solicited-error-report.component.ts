...
export class SolicitedErrorReportComponent {
  ...
  Fetch() {
    ...
    const url = `${environment.apiBaseUrl}/api/SolicitedErrReport?fromdate=${txt_FromDate}&todate=${txt_ToDate}&reportuser=${this.loginUser}`;
      ...
      next: (res) => {
        if (res && res.length > 0) {
          if (rhb_Screen === 'S' && res.length < 1000) {
            ...
            this.totalPages = Math.ceil(this.Solerr.length / this.itemsPerPage);
  ...
  export(res: any, file: any) {
    ...
    this.excelService.exportToFile(
      this.GridData, 'Solicited Error Report', {
      mobile: 'Mobile #',
      nprdate: 'NPR Date',
      username: 'Name',
