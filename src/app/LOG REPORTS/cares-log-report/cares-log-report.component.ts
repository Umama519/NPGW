...
export class CaresLogReportComponent {
  constructor(private http: HttpClient, private el: ElementRef, private pager: PaginationService, private renderer: Renderer2, private datepickerService: DatepickerService, private excelService: ExcelExportService) {
      ...
      next: (res) => {
        if (res && res.length > 0) {
          if (rhb_Screen === 'S' && res.length < 1000) {
            ...
            this.totalPages = Math.ceil(this.Cares.length / this.itemsPerPage);
