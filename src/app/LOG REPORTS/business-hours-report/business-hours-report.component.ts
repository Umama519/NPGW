...
export interface BuisnessHours{
  mobile: string;
  msgid: string;
  portid: string;
  arrivaldate: string;
...
export class BusinessHoursReportComponent {
constructor(private http: HttpClient, private el: ElementRef, private renderer: Renderer2, private datepickerService: DatepickerService, private excelService: ExcelExportService) { }
    ...
    Fetch() {
      ...
      const url = `${environment.apiBaseUrl}/api/BuisnessHoursReport?fromdate=${txt_FromDate}&todate=${txt_ToDate}&userid=${this.loginUser}`;
    ...
    export(res:any, file:any) {
      ...
      this.excelService.exportToFile( 
        this.GridData, 'Business Hours Message Report', {
        mobile: 'Mobile #',
        msg: 'Message',
        msgid: 'Message ID',
        portid: 'Port ID',
