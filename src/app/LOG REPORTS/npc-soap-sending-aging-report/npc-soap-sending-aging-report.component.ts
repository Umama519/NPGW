...
export interface NPCSOAPSending{
  mobile: string;
  msg: string;
  arrivaldate: string;
  portdate: string;
...
@Component({
  selector: 'app-report-npcsoapsendingrpt-aspx',
<<<<<<< HEAD
<<<<<<< HEAD
  standalone: true,
=======
...
>>>>>>> df6adcae315ddb201abfceb949d6603edc257f9e
  ...
  templateUrl: './npc-soap-sending-aging-report.component.html',
  styleUrl: './npc-soap-sending-aging-report.component.css'
})
export class NpcSoapSendingAgingReportComponent {
constructor(private http: HttpClient, private el: ElementRef, private renderer: Renderer2, private datepickerService: DatepickerService, private excelService: ExcelExportService) { }
    ...
    Fetch() {
      ...
      const url = `${environment.apiBaseUrl}/api/NPCSOAPSendingReport?fromdate=${txt_FromDate}&todate=${txt_ToDate}&userid=${this.loginUser}`;
    ...
    export(res:any, file:any) {
      ...
      this.excelService.exportToFile( 
        this.GridData, 'NPC SOAP Sending Report', {
        mobile: 'Mobile #',
        msg: 'Message',
        portdate: 'Port Date',
        duedate: 'Due Date',
