...
export class NprCurrentBulkStatusComponent {
  ...
  GetGrid() {
    debugger;
    const url = `${environment.apiBaseUrl}/api/NPRCurrentStatus/S?id=${this.loginUser}`;
  ...
  GetGrid1() {
    debugger;
    const url = `${environment.apiBaseUrl}/api/NPRCurrentStatus/BD?id=${this.loginUser}`;
    this.http.get<any[]>(url).subscribe({
      next: (res: any[]) => {
        if (res && res.length > 0) {
          this.GridData1 = res;
  ...
  GetGrid2(batch: any) {
    debugger;
    const url = `${environment.apiBaseUrl}/api/NPRManualBilk/BD?userid=${this.loginUser}&batch=${batch}`;
    this.http.get<any[]>(url).subscribe({
      next: (res: any[]) => {
        if (res && res.length > 0) {
          this.GridData2 = res;
  ...
  submit(rowData: any) {
    ...
    if (this.isadd === 'I') {
      ...
      const insertUrl = `${environment.apiBaseUrl}/api/NPRCurrentStatus/I`;
      this.http.post(insertUrl, bulkData).subscribe({
        next: (response: any) => {
          ...
          //const mappedData = setup;
          //const insertUrl1 = `${environment.apiBaseUrl}/api/NPRBulkStatus`;
  ...
  deleteRecord() {
    ...
    const url = `${environment.apiBaseUrl}/api/NPRCurrentStatus/${this.loginUser}`
    const options = {
  ...
  export(res: any, file: any) {
    ...
    this.excelService.exportToFile(
      res, 'Bulk NPR Current Status',
      {
        ...
        secuirty_deposit: 'Security Deposit',
        line_rent: 'Line Rent',
