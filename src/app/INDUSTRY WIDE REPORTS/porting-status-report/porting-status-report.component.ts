...
export class PortingStatusReportComponent {
constructor(private http: HttpClient, private el: ElementRef, private renderer: Renderer2, private datepickerService: DatepickerService, private excelService: ExcelExportService) { }
  ...
  Export = [
    ...
    Fetch() {
      ...
      const url = `${environment.apiBaseUrl}/api/PortingStatusReport?fromdate=${txt_FromDate}&todate=${txt_ToDate}&userid=${this.loginUser}`;
      ...
      this.http.get<any>(url).subscribe({
        next: (res) => {
          if (res && res.length > 0) {
            if (rhb_Screen === 'S') {
              this.Porting = res;
              this.Porting = this.Porting.map(x => ({
      ...x,
      difference: Number(x.recepient) - Number(x.donor)
    }));
            } else {
              setTimeout(() => {
